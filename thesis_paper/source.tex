...
\item[5.] Get everything block shared memory to global memory
\end{itemize}
...
As we already mentioned, the main purpose of our proposal is the system that allows integration of GPU into push-based system, thus, we didn't go with too sophisticated and optimized implementations for each query that we benchmark, assuming that user's wouldn't do it and having more or less realistic speedup results. Figure \ref{fg:sharedMem} shows swimlanes as execution thread for blocks. Every row is an execution block for an atom, where first element is the mapped atom. First of all, you can observe that we can clearly see that the execution threads are almost equally balanced (exceptions on odd number of atoms). Every swimlane (bordered with black) represent within block execution. Colored numbers are the atoms that we loaded into shared memory. Of course, this is about half of improvement, and we can happily add improvement of adding second part too, but, at this point, we go with this average improvement approach.
...
\State \Comment{ $atoms$ is vector of atoms kept in global gpu memory}
\State \Comment{ $atomsCount$ is number of atoms in the frame kept in global gpu memory}
\State \Comment{ $histogram$ is buckets with counters for the SDH}
\State \Comment{ $pdhRes$ is resolution of histogram}
\State \Comment{ $numOfBuckets$ is number of buckets}

\BState \emph{top}:

\State $\textit{smem} \gets \textit{allocate shared memory in each GPU block for fast write access of histogram buckets}$
...
\State $\textit{indexX and indexY} \gets \textit{calculate from grid sizes}$ 
\State $\textit{index} \gets \textit{get atom mapped to this kernel}$
\State \Comment{Finally get the index of current atom mapped to this kernel}
...
\State $\textit{k} \gets \textit{0}$
\State $\textit{i} \gets \textit{index}$
...
\State $\textit{end} \gets \textit{increment by one if it is odd and total count is even}$

\State $\textit{bi} \gets \textit{blockDim.x * blockIdx.x}$
\State $\textit{ei} \gets \textit{bi + blockDim.x}$
