...
\noindent\hspace{3em}As a plugin for optimizing computation of scientific queries against large data sets, an improved Push-based System for MS improvement should be able to handle data streams as well, in order to get real time results and saving up on time, instead of pulling everything from files later on.
...
\State $\textit{sharedResult} \gets \textit{pointer to shared memory head}$
...
\State
\State $\textit{// Here we will starting collecting information from each block}$
\If {$\textit{threadIdx.x} = 0$}
\State  $\textit{atomicAdd(\&results->mass, sharedResult[0]);}$ \Comment{All atomic functions here}
...
\noindent\hspace{3em} Implementation for CPU version of SDH is very simple, we do 2 nested loops that calculate histogram. Since SDH criteria and parameters might vary by bucket width and box size for the simulation space, we've used one sample size for most of them, though results don't change a lot for sequential computation.
